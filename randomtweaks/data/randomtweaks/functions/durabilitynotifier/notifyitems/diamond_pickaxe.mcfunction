execute as @s[nbt={SelectedItem:{id:"minecraft:diamond_pickaxe"}}] run tag @s add rtw.success
execute as @s[tag=rtw.success] store result score @s plib.pc.value run data get entity @s SelectedItem.tag.Damage
execute as @s[tag=rtw.success] run scoreboard players set @s plib.pc.maxn 1561
